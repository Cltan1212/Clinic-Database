...
CREATE TABLE tenant (
    ...
    tenant_street  VARCHAR2(40) NOT NULL,
    tenant_city    VARCHAR2(30) NOT NULL,
    tenant_state   CHAR(3) NOT NULL,
    tenant_pcode   CHAR(4) NOT NULL,
    tenant_phone   CHAR(10)
);
...
COMMENT ON COLUMN tenant.tenant_street IS
    'Contact street address for tenant';

COMMENT ON COLUMN tenant.tenant_city IS
    'Contact city for tenant';

COMMENT ON COLUMN tenant.tenant_state IS
    'Contact state address for tenant';

COMMENT ON COLUMN tenant.tenant_pcode IS
    'Contact postcode for tenant';
...
ALTER TABLE damage
    ADD CONSTRAINT damage_payment_fk FOREIGN KEY ( pay_no )
        REFERENCES payment ( pay_no );

ALTER TABLE property
    ADD CONSTRAINT owner_property_fk FOREIGN KEY ( owner_no )
        REFERENCES owner ( owner_no );

ALTER TABLE payment
    ADD CONSTRAINT paymethod_payment_fk FOREIGN KEY ( pay_paidby )
        REFERENCES paymethod ( paymethod_id );

ALTER TABLE maintenance
    ADD CONSTRAINT property_maintenance_fk FOREIGN KEY ( prop_no )
        REFERENCES property ( prop_no );

ALTER TABLE rent
    ADD CONSTRAINT property_rent_fk FOREIGN KEY ( prop_no )
        REFERENCES property ( prop_no );

ALTER TABLE damage
    ADD CONSTRAINT rent_damage_fk FOREIGN KEY ( rent_agreement_no )
        REFERENCES rent ( rent_agreement_no );

ALTER TABLE payment
    ADD CONSTRAINT rent_payment_fk FOREIGN KEY ( rent_agreement_no )
        REFERENCES rent ( rent_agreement_no );

ALTER TABLE rent
    ADD CONSTRAINT tenant_rent_fk FOREIGN KEY ( tenant_no )
